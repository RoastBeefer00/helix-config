...
(provide open-term
         ...
         ;; Exported for use by external terminal-based plugins (lazygit, sidekick, etc.)
         make-terminal
         make-terminal-custom
         make-terminal-with-renderer
         make-terminal-renderer
         show-term
         stop-terminal
         term-resize-from-term
...
;; Base constructor — accepts both a custom renderer and a custom event handler.
;; External packages (lazygit, sidekick, etc.) use this to compose terminals.
(define (make-terminal-custom name shell rows cols on-start-func callback-function renderer event-handler)
...
(define (make-terminal-with-renderer name shell rows cols on-start-func callback-function renderer)
  (make-terminal-custom name shell rows cols on-start-func callback-function renderer terminal-event-handler))

(define (make-terminal name shell rows cols on-start-func callback-function)
  (make-terminal-custom name shell rows cols on-start-func callback-function terminal-render terminal-event-handler))
...
;; Factory that produces a terminal renderer using the given area-calculation function.
;; calculate-area-fn :: (state rect) -> area
(define (make-terminal-renderer calculate-area-fn)
  (lambda (state rect frame)
    (unless (unbox (Terminal-kill-switch state))
      (define block-area (calculate-area-fn state rect))
...
(define terminal-render (make-terminal-renderer alternative-calculate-area))
    ...
    [(unbox (Terminal-focused? state))
     ...
     (cond
       ...
       [char
        (if (equal? (key-event-modifier event) key-modifier-ctrl)
            (pty-process-send-command *pty-process*
              (string (integer->char (max 1 (- (char->integer (char-upcase char)) 64)))))
            (pty-process-send-command-char *pty-process* char))
        event-result/consume]
