...
(define (%process-binding! conf binding)
  (define key-str (%key->str (car binding)))
  (define sub (cdr binding))
  (cond
    [(and (pair? sub) (null? (cdr sub)) (string? (car sub)))
     (hash-insert conf key-str (car sub))]
    [(and (pair? sub) (null? (cdr sub)) (symbol? (car sub)))
     (hash-insert conf key-str (symbol->string (car sub)))]
    [else
     (hash-insert conf key-str (%build-submap sub))]))

(define (%build-submap bindings)
  (let loop ([acc (hash)] [bs bindings])
    (if (null? bs)
        acc
        (loop (%process-binding! acc (car bs))
              (cdr bs)))))
...
(define-syntax #%keybindings
  (syntax-rules ()
    [(_ conf) conf]
    [(_ conf binding)
     (%process-binding! conf (quote binding))]
    [(_ conf binding rest ...)
     (#%keybindings (%process-binding! conf (quote binding)) rest ...)]))
